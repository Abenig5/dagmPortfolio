    ...
    <section id="projects" className="relative overflow-hidden bg-black py-32">
      ...
      <Container className="relative z-10">
        <div className="mb-20">
          ...
          <h2 className="mb-6 text-5xl font-bold leading-tight text-on-surface sm:text-6xl lg:text-7xl">
  ...
  return (
    <TiltCard className="glass-card tilt-card group relative overflow-hidden rounded-xl will-change-transform">
      <div className="grid grid-cols-1 lg:grid-cols-12">
        <div className={`h-[320px] overflow-hidden md:h-[500px] lg:col-span-7 ${reversed ? "lg:order-2" : ""}`}>
          <ProjectVisual project={project} />
        </div>
        <div className={`flex flex-col justify-center p-8 md:p-12 lg:col-span-5 ${reversed ? "lg:order-1" : ""}`}>
          ...
          <h3 className="mb-4 text-4xl font-semibold leading-tight text-on-surface">{project.title}</h3>
...
const ProjectVisual = ({ project }: ProjectVisualProps) => {
  ...
  return (
    <div className={`project-visual ${visualClass} relative flex h-full items-center justify-center overflow-hidden grayscale transition duration-700 group-hover:scale-105 group-hover:grayscale-0`}>
      ...
      <div className="absolute left-10 top-10 font-mono text-xs uppercase tracking-widest text-white/60">{project.category}</div>
      <div className="absolute bottom-10 left-10 right-10 rounded-xl border border-white/10 bg-black/30 p-4 backdrop-blur">
